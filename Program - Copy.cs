...
namespace XboxDebugConsole
{
    ...
    internal class Application
    {
        ...
        private static string? GetArgString(string subcommand, string[] args)
        {
            for (int i = 0; i < args.Length; i++)
            {
                // Check if this arg starts with "subcommand="
        ...
        private static int GetArgInt(string subcommand, string[] args, int defaultValue = 0)
        {
            string? strValue = GetArgString(subcommand, args);
        ...
        private static bool GetArgBool(string subcommand, string[] args, bool defaultValue = false)
        {
            string? strValue = GetArgString(subcommand, args);
        ...
        private static void ProcessInput(string input)
        {
            ...
            string? ipAddress   = GetArgString("ip", args);
            string? localPath   = GetArgString("localPath", args);
            string? pdbPath     = GetArgString("pdbPath", args);
            string? remotePath  = GetArgString("remotePath", args);
            string? address     = GetArgString("address", args);
            string? data        = GetArgString("data", args);
            string? xboxName    = GetArgString("name", args);

            int timeoutMs = GetArgInt("timeoutMs", args, 5000);
            int length = GetArgInt("length", args, 0);
            int threadId = GetArgInt("threadId", args, 0);

            bool autoReconnect = GetArgBool("autoReconnect", args, true);
        ...
        {
            var cmd = JObject.Parse(input);
            string? commandStr = cmd["command"]?.ToString().ToLower();

            if (string.IsNullOrEmpty(commandStr))
            {
                WriteError(Command.Unknown, "Command field is required.");
                return;
            }
            ...
            string? ipAddress   = cmd["ip"]?.ToString();
            string? localPath   = cmd["localPath"]?.ToString();
            string? remotePath  = cmd["remotePath"]?.ToString();
            string? pdbPath     = cmd["pdbPath"]?.ToString();
            string? address     = cmd["address"]?.ToString();
            string? data        = cmd["data"]?.ToString();
            string? xboxName    = cmd["name"]?.ToString();

            int timeoutMs = cmd["timeoutMs"]?.Value<int>() ?? 5000;
            int length = cmd["length"]?.Value<int>() ?? 0;
            int threadId = cmd["threadId"]?.Value<int>() ?? 0;

            bool autoReconnect = cmd["autoReconnect"]?.Value<bool>() ?? true;
            ...
            int timeoutMs, int length, int threadId, bool autoReconnect)
        {
            ...
            {
                case Command.Scan:
                    Scan(command, timeoutMs);
                    break;

                case Command.Connect:
                    Connect(command, ipAddress, xboxName, timeoutMs);
                    break;
                ...
                case Command.Mute:
                    _NotificationsMuted = true;
                    WriteSuccess(command, "Notifications muted.");
                    break;

                case Command.Unmute:
                    _NotificationsMuted = false;
                    WriteSuccess(command, "Notifications unmuted.");
                    break;

                case Command.SetBreakpoint:
                    if (string.IsNullOrEmpty(address))
                    {
                        WriteError(command, "address is required for setBreak command.");
                        break;
                    }

                    SetBreakpoint(command, address);
                    break;

                case Command.DeleteBreakpoint:
                    if (string.IsNullOrEmpty(address))
                    {
                        WriteError(command, "address is required for deleteBreak command.");
                        break;
                    }

                    DeleteBreakpoint(command, address);
                    break;
                ...
                case Command.ReadMemory:
                    if (string.IsNullOrEmpty(address) || length <= 0)
                    {
                        WriteError(command, "address and length are required for read command.");
                        break;
                    }

                    ReadMemory(command, address, length);
                    break;
                ...
                case Command.DumpMemory:
                    ...
                    {
                        WriteError(command, "address, length, and localPath are required for dump command.");
                        break;
                    }

                    DumpMemory(command, address, length, localPath);
                    break;
                ...
                case Command.WriteMemory:
                    ...
                    {
                        WriteError(command, "address and data are required for write command.");
                        break;
                    }

                    WriteMemory(command, address, data);
                    break;

                case Command.Registers:
                    GetRegisters(command, threadId);
                    break;
                ...
                case Command.Upload:
                    ...
                    {
                        WriteError(command, "localPath and remotePath are required for upload command.");
                        break;
                    }

                    UploadFile(command, localPath, remotePath);
                    break;

                case Command.Launch:
                    if (string.IsNullOrEmpty(remotePath))
                    {
                        WriteError(command, "remotePath is required for launch command.");
                        break;
                    }

                    LaunchFile(command, remotePath);
                    break;

                case Command.LaunchDash:
                    LaunchFile(command, _DashPath);
                    break;

                case Command.Reboot:
                    Reboot(command, autoReconnect, timeoutMs);
                    break;
                ...
                case Command.Help:
                    ...
                    {
                        WriteError(command, "Help command is not available in JSON mode.");
                    }
                    break;
